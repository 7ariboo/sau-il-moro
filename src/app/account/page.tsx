...
function AuthForms({
  ...
  const handleOAuth = async (provider: 'google' | 'apple') => {
    ...
    if (!result.success) {
      setError(result.error || `Errore durante l'accesso con ${provider}`);
    }
    setIsSubmitting(false);
  };
        ...
        <div className="w-full max-w-md bg-white p-8 border border-gray-100 shadow-sm">
          ...
          <div className="space-y-3 mb-6">
            <button
              ...
              className="w-full flex items-center justify-center gap-3 bg-white border border-gray-300 py-3 px-4 text-xs font-bold text-gray-700 hover:bg-gray-50 transition-colors uppercase tracking-wider"
            >
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-3 bg-black text-white py-3 px-4 text-xs font-bold hover:bg-gray-900 transition-colors uppercase tracking-wider"
            >
              <svg width="18" height="18" fill="currentColor" viewBox="0 0 24 24"><path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.81-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M15.97 6.85c.67-.82 1.13-1.96.99-3.1-.97.04-2.17.65-2.87 1.46-.62.72-1.16 1.88-1.01 3.01 1.09.08 2.22-.55 2.89-1.37z"/></svg>
              Accedi con Apple
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <ButtonCustom type="submit" className="w-full h-14" disabled={isSubmitting}>
              {isSubmitting ? 'Attendere...' : mode === 'login' ? 'Accedi al Profilo' : 'Crea Account Ufficiale'}
            </ButtonCustom>
          </form>
        </div>
      </div>
